...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
const uint8_t aTxBuffer = 0xAA;
uint8_t aRxBuffer[16] __attribute__((section("noncacheable_buffer")));
...
int main(void)
{
...
#if 1
  if (HAL_PSSI_Receive_DMA(&hpssi, (uint32_t *)aRxBuffer, 4) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  if (HAL_PSSI_Receive(&hpssi, (uint32_t *)&aRxBuffer, 4, 0xFFF) != HAL_OK)
  {
    Error_Handler();
  }
#endif
